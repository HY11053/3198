...
class ArticleController extends Controller
{
    ...
    public function BrandArticle(Request $request,$id){
        ...
        $tongleibrands=Cache::remember('tongleibrands'.$thisArticleInfos->typeid,  config('app.cachetime')+rand(60,60*24), function() use ($thisArticleInfos){
            return Brandarticle::where('typeid',$thisArticleInfos->typeid)->skip(10)->take(6)->orderBy('click','desc')->get(['id','litpic','brandname']);
        });
        $paihangbangs=Cache::remember('thisArticleInfosphb'.$thisArticleInfos->typeid,  config('app.cachetime')+rand(60,60*24), function() use ($thisArticleInfos){
    ...
    public function newsArticle(Request $request,$id){
        ...
        if ($thisArticleInfos->brandid && Brandarticle::where('id',$thisArticleInfos->brandid)->value('id')){
            ...
            $thisArticleTypeInfo=Cache::remember('thisArticleTypeInfo'.$thisBrandArticleInfos->typeid,  config('app.cachetime')+rand(60,60*24), function() use ($thisBrandArticleInfos){
                return Arctype::where('id',$thisBrandArticleInfos->typeid)->first(['id','typename','reid']);
            });
            $thisArticleTopTypeInfo=Cache::remember('thisArticleTopTypeInfo'.$thisArticleTypeInfo->reid,  config('app.cachetime')+rand(60,60*24), function() use ($thisArticleTypeInfo){
                return Arctype::where('id',$thisArticleTypeInfo->reid)->first(['id','typename','real_path']);
            });
            $tongleibrands=Cache::remember('tongleibrands'.$thisBrandArticleInfos->typeid,  config('app.cachetime')+rand(60,60*24), function() use ($thisBrandArticleInfos){
                return Brandarticle::where('typeid',$thisBrandArticleInfos->typeid)->skip(10)->take(6)->orderBy('click','desc')->get(['id','litpic','brandname']);
            });
            $paihangbangs=Cache::remember('thisArticleInfosphb'.$thisBrandArticleInfos->typeid,  config('app.cachetime')+rand(60,60*24), function() use ($thisBrandArticleInfos){
